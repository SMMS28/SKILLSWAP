...
class MigrationManager {
  ...
  async seed() {
    ...
    try {
      ...
      // 4. Insert user_skills_wanted
      const skillsWanted = [
        { userID: dbUsers[0].id, skillID: dbSkills[2].id, skillLevel: 'Intermediate', maxHourlyRate: 40, description: 'Want to learn photography for travel' },
        { userID: dbUsers[0].id, skillID: dbSkills[7].id, skillLevel: 'Beginner', maxHourlyRate: 20, description: 'Interested in growing vegetables' },
        { userID: dbUsers[1].id, skillID: dbSkills[0].id, skillLevel: 'Beginner', maxHourlyRate: 45, description: 'Want to learn web development' },
        { userID: dbUsers[2].id, skillID: dbSkills[5].id, skillLevel: 'Beginner', maxHourlyRate: 30, description: 'Looking to start yoga practice' },
        { userID: dbUsers[3].id, skillID: dbSkills[3].id, skillLevel: 'Beginner', maxHourlyRate: 25, description: 'Always wanted to play guitar' },
        { userID: dbUsers[4].id, skillID: dbSkills[6].id, skillLevel: 'Intermediate', maxHourlyRate: 50, description: 'Need to improve my web design skills' }
      ];

      for (const wanted of skillsWanted) {
        await this.db.run(
          `INSERT OR IGNORE INTO user_skills_wanted (id, userID, skillID, skillLevel, maxHourlyRate, description) 
           VALUES (?, ?, ?, ?, ?, ?)`,
          [uuidv4(), wanted.userID, wanted.skillID, wanted.skillLevel, wanted.maxHourlyRate, wanted.description]
        );
      }
