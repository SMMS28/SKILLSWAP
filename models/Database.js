...
class DatabaseService {
  ...
  async addUserSkillWanted(userID, skillID, skillLevel, maxHourlyRate, description) {
    const id = uuidv4();
    await this.db.run(
      `INSERT OR REPLACE INTO user_skills_wanted (id, userID, skillID, skillLevel, maxHourlyRate, description) 
       VALUES (?, ?, ?, ?, ?, ?)`,
      [id, userID, skillID, skillLevel, maxHourlyRate, description]
    );
    return { id, userID, skillID, skillLevel, maxHourlyRate, description };
  }
  ...
  async getUserSkillsWanted(userID) {
    return await this.db.query(`
      SELECT usw.*, s.name as skillName, s.category 
      FROM user_skills_wanted usw 
      JOIN skills s ON usw.skillID = s.id 
      WHERE usw.userID = ?
    `, [userID]);
  }
  ...
  async addMessage(messageData) {
    ...
    await this.db.run(
      `INSERT INTO messages (id, exchangeID, senderID, content, messageType) 
       VALUES (?, ?, ?, ?, ?)`,
      [id, exchangeID, senderID, content, messageType || 'text']
    );

    return { id, ...messageData, createdAt: new Date().toISOString() };
  }
  ...
  async getUserTransactions(userID) {
    return await this.db.query(`
      SELECT t.*, e.skill as exchangeSkill
      FROM transactions t
  ...
  async getSearchFilters() {
    ...
    return {
      ...
      skillLevels,
      maxHourlyRate: 1000 // This could be calculated from actual data
    };
  }
