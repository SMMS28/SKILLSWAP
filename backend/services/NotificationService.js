...
class NotificationService {
  ...
  async notifyUsersBySkill(skillId, type, title, message, data = {}) {
    try {
      const users = await database.db.query(`
        SELECT DISTINCT u.id 
        FROM users u
        JOIN user_skills_wanted usw ON u.id = usw.userID
        WHERE usw.skillID = ?
      `, [skillId]);
      ...
      console.error('Error sending skill-based notifications:', error);
      throw error;
    }
  }
}
